...
import nflx from '../stock_json/nflx.json'

const nflxData = 'https://yahoo-finance-api.vercel.app/NFLX';
async function getData() {
    const response = await fetch(nflxData);
    return response.json();
}

const API_TOKEN = "c9mrn1qad3ie533htcn0";
const STOCK_SYMBOLS = ["NFLX"]
...
function Netflix() {
    ...
    function addPredictionData(data) {
        ...
        for(let i = 0; i < 30; i++) {
            predicted.push(nflx.c[i]);
            predTimestamps.push(nflx.t[i]);
        }
        data.pc = predicted;
        data.pt = predTimestamps;
        // for(let i = 0; i < 30; i++) {
        //     data.c.push(nflx.c[i]);
        //     data.t.push(nflx.t[i]);
        // }
    }
    ...
    useEffect(() => {
        ...
        async function getLatestPrice() {
            try {
                ...
                console.log(data);
                const nflx = data.chart.result[0];
                setPrevPrice(price);
                setPrice(nflx.meta.regularMarketPrice.toFixed(2));
                setPriceTime(new Date(nflx.meta.regularMarketTime * 1000));
                const quote = nflx.indicators.quote[0];
                const prices = nflx.timestamp.map((timestamp, index) => ({
        ...
        <div>
            ...
            <div className="container">
                <div className="selector">
                    <strong>Netflix Historical Data and Predictions</strong><br/>
            ...
            <ResponsiveContainer width="100%" height={300}>
                <LineChart data={data}
                    ...
                    <YAxis type="number" allowDecimals={true}
                           allowDataOverflow={true} domain={[150, 550]}/>
                    <Tooltip />
                    <Legend />
