...
import fb from '../stock_json/fb.json'

const fbData = 'https://yahoo-finance-api.vercel.app/FB';
async function getData() {
    const response = await fetch(fbData);
    return response.json();
}

const API_TOKEN = "c9mrn1qad3ie533htcn0";
const STOCK_SYMBOLS = ["FB"]
...
function Facebook() {
    ...
    function addPredictionData(data) {
        ...
        for(let i = 0; i < 30; i++) {
            predicted.push(fb.c[i]);
            predTimestamps.push(fb.t[i]);
        }
        data.pc = predicted;
    ...
    useEffect(() => {
        ...
        async function getLatestPrice() {
            try {
                ...
                console.log(data);
                const fb = data.chart.result[0];
                setPrevPrice(price);
                setPrice(fb.meta.regularMarketPrice.toFixed(2));
                setPriceTime(new Date(fb.meta.regularMarketTime * 1000));
                const quote = fb.indicators.quote[0];
                const prices = fb.timestamp.map((timestamp, index) => ({
        ...
        <div>
            ...
            <div className="container">
                <div className="selector">
                    <strong>Facebook Historical Data and Predictions</strong><br/>
            ...
            <ResponsiveContainer width="100%" height={300}>
                <LineChart data={data}
                    ...
                    <YAxis type="number" allowDecimals={true}
                           allowDataOverflow={true} domain={[150, 350]}/>
                    <Tooltip />
                    <Legend />
